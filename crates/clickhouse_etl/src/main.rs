use anyhow::Result;
use clickhouse_etl::{
    ClickHouseConfig, ClickHouseLoader, EtlTransformer, GgTxtParser, DEFAULT_HAND_BATCH_SIZE,
};
use rayon::prelude::*;
use std::env;
...
async fn main() -> Result<()> {
    ...
    if args.len() < 6 {
        eprintln!(
            "Usage: clickhouse_etl <input_dir> <clickhouse_url> <database> <user> <password>"
        );
        return Ok(());
    }

    let input_dir = PathBuf::from(&args[1]);
    let loader = ClickHouseLoader::new(ClickHouseConfig::new(
